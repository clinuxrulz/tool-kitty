...
import { makeDefaultViaTypeSchema, } from "tool-kitty-type-schema";
...
export class AddNodeMode<TYPE_EXT,INST_EXT> implements Mode {
  instructions: Component;
  overlayHtmlUi: Component;
  sideForm: Accessor<Component | undefined>;

  constructor(modeParams: ModeParams<TYPE_EXT,INST_EXT>) {
    let [ state, setState ] = createStore<{
      pan: Vec2,
      scale: number,
      formMousePos: Vec2 | undefined,
      dragging: NoTrack<{
        nodeType: NodeType<TYPE_EXT,INST_EXT,any>,
        nodeSize: Vec2,
        pickupOffset: Vec2,
      }> | undefined,
    }>({
    ...
    let atY = 0.0;
    for (let nodeType of modeParams.nodeRegistry.nodeTypes) {
      atY -= 80;
      world.createEntity([
    ...
    let nodesSystem = new NodesSystem<TYPE_EXT,INST_EXT>({
      nodeRegistry: modeParams.nodeRegistry,
      world: () => world,
    });
