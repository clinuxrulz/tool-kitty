...
import { intersectionComponentType, IntersectionState } from "../components/IntersectionComponent";
import { glsl } from "@bigmistqke/view.gl/tag";

export class IntersectionNodeType implements NodeType<NodeTypeExt,NodeExt,IntersectionState> {
  componentType = intersectionComponentType;
  ext: NodeTypeExt = {};

  create(nodeParams: NodeParams<IntersectionState>) {
    return new IntersectionNode(nodeParams);
  }
}

export const intersectionNodeType = new IntersectionNodeType();

class IntersectionNode implements Node<NodeTypeExt,NodeExt,IntersectionState> {
  type = intersectionNodeType;
  nodeParams: NodeParams<IntersectionState>;
  ...
  constructor(nodeParams: NodeParams<IntersectionState>) {
    ...
    this.ext.generateCode = ({ ctx, inputs, }) => {
      ...
      if (k_?.type == "Atom") {
        ...
        ctx.insertGlobalCode(glsl`
          float ${sdfFn}(vec3 p) {
            return max(${model1.sdfFuncName}(p), ${model2.sdfFuncName}(p));
          }
        `);
      }
