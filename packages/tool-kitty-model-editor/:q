...
import { xorComponentType, XorState } from "../components/XorComponent";
import { glsl } from "@bigmistqke/view.gl/tag";

export class XorNodeType implements NodeType<NodeTypeExt,NodeExt,XorState> {
  componentType = xorComponentType;
  ext: NodeTypeExt = {};

  create(nodeParams: NodeParams<XorState>) {
    return new XorNode(nodeParams);
  }
}

export const xorNodeType = new XorNodeType();

class XorNode implements Node<NodeTypeExt,NodeExt,XorState> {
  type = xorNodeType;
  nodeParams: NodeParams<XorState>;
  ...
  constructor(nodeParams: NodeParams<XorState>) {
